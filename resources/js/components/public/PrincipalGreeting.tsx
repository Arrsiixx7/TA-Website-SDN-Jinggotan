import { Teacher, SchoolProfile } from '@/types';
...
export default function PrincipalGreeting({
    ...
    // Use principal if available, otherwise show default
    const principalName = principal?.name || 'Ely Kusrini, S.Pd.';
    const principalPosition =
        principal?.position || 'Kepala SD Negeri Jinggotan';
    ...
    return (
        <section className="bg-linear-to-b from-primary/5 via-white to-white py-24">
            <div className="mx-auto grid max-w-7xl items-center gap-16 px-6 lg:grid-cols-2">
                {/* Image */}
                <div className="flex justify-center lg:justify-start">
                    <div className="group relative">
                        <div className="absolute -inset-4 rounded-3xl bg-primary/20 opacity-60 blur-2xl transition duration-500 group-hover:opacity-80"></div>
                        <img
                            src={principalImage}
                            alt={principalName}
                ...
                <div className="relative">
                    ...
                    <div className="my-6 h-1.5 w-24 rounded-full bg-primary"></div>
                    <p className="mb-8 text-lg leading-8 text-gray-600">
                        {formatGreeting(greetingText)}
                    </p>
